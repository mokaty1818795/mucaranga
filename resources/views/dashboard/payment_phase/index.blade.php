                ...
                <div class="card">
                    <div class="card-header row justify-content-between">
                        ...
                        <a href="{{ route('payment_phase.create') }}" class="col-auto btn btn-purple">
                            @svg('fluentui-payment-16','feather align-middle') &nbsp; <span class="align-middle">Nova fase
                                de pagamento</span>
                        </a>
                    </div>
                </div>
                                ...
                                @foreach ($payment_phases as $payment_phase)
                                    <tr>
                                        ...
                                        <td><button class="btn btn-danger"
                                                onclick="document.getElementById('payment_phase_{{ $payment_phase->id }}_delete').submit()">Delete</button>
                                            <form action="{{ route('payment_phase.destroy', $payment_phase->id) }}"
                                                method="post" id="payment_phase_{{ $payment_phase->id }}_delete">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                        </td>
